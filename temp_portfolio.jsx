                ...
                <tr key={c.id} 
                  ...
                  {allModifiers.map(modName => {
                    ...
                    return (
                      <td key={modName}
                          onClick={(e) => {
                            if (mod && onHeatmapClick) {
                              e.stopPropagation();
                              const modIndex = mList.findIndex(m => (m.name || m.modifier_name) === modName);
                              onHeatmapClick(c.id, modIndex);
                            }
                          }}
                          title={tooltip}
                          onMouseEnter={() => setHoveredMod(modName)}
                          onMouseLeave={() => setHoveredMod(null)}
                          style={{ padding: '4px', borderBottom: '1px solid #F1F5F9', textAlign: 'center', background: hoveredMod === modName ? '#F8FAFC' : 'transparent', transition: 'background 0.2s', cursor: mod ? 'pointer' : 'default' }}>
