...
export default function EvidenceSources({ hasRun }) {
  const sources = [
    { id: 1, name: 'SEC Collector', type: 'Financial Database', icon: <Building2 size={20} color="var(--accent-blue)" />, contributed: 'Revenue figures, SEC filings structure, Subsidiaries data.' },
    { id: 2, name: 'Wikipedia', type: 'Public Knowledge Graph', icon: <BookOpen size={20} color="var(--accent-blue)" />, contributed: 'General company profile, History, Overview summaries.' },
    { id: 3, name: 'Wikidata', type: 'Structured Fact DB', icon: <Database size={20} color="var(--accent-blue)" />, contributed: 'Acquisitions, Geographic spread, HQ location, SIC Codes.' },
    { id: 4, name: 'DB Collector', type: 'Business Intelligence', icon: <Search size={20} color="var(--accent-blue)" />, contributed: 'Industry classification, B2B/B2C indicators, Employee count.' },
    { id: 5, name: 'Domain Scraper', type: 'Live Web Scraping', icon: <Globe size={20} color="var(--accent-blue)" />, contributed: 'HTTPS encryption status, E-commerce signals, Privacy policies.' }
  ];
        ...
        {sources.map(source => (
          <div key={source.id} style={{
            ...
            <div style={{ fontSize: '0.8rem', color: 'var(--text-secondary)', lineHeight: '1.4', marginTop: 'auto', paddingTop: '12px', borderTop: '1px dashed rgba(255,255,255,0.1)' }}>
              <strong>Contributed:</strong> {source.contributed}
            </div>
          </div>
        ))}
      </div>
